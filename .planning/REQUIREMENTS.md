...
- [x] **CORE-02**: Library user can implement `Behavior` trait with `name() -> &'static str`, `order_key() -> K`, `evaluate(&I, &S) -> BehaviorResult<D, O>`
- [x] **CORE-03**: `BehaviorResult<D, O>` provides `Continue(Vec<D>)` and `Stop(O)` variants
- [x] **CORE-04**: `Outcome<F, N>` enum provides `Committed(F)`, `Undone(F)`, `Redone(F)`, `NoChange`, `InvalidInput(N)`, `Disallowed(N)`, `Aborted(N)`
- [x] **CORE-05**: `EngineError` is distinct from `Outcome` — reserved for engine-internal failures only
...
- [x] **DISP-02**: `dispatch(input, reversibility)` evaluates behaviors in deterministic `(order_key, name)` order, applies diffs immediately, appends trace at moment of diff application, commits `Frame` atomically if non-empty
- [x] **DISP-03**: `Frame<I, D, T>` stores `input`, `diff` collection, and `trace` as canonical committed record
...
- [x] **HIST-03**: Committing an irreversible transition erases both undo and redo stacks
- [x] **HIST-04**: Undo stores a full state snapshot per frame (no reverse-diff trait requirement on user types)
