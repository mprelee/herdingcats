...
- [ ] **REN-01**: `Operation<S>` trait renamed to `Mutation<S>` — `apply`, `undo`, `hash_bytes` method signatures preserved
- [ ] **REN-02**: `Rule<S,O,E,P>` trait renamed to `Behavior<S,M,I,P>` — `before`, `after`, `id`, `priority` method signatures preserved
- [ ] **REN-03**: `Transaction<O>` renamed to `Action<M>` — `mutations` vec (was `ops`), `deterministic`, `cancelled` fields preserved; `irreversible` field removed
